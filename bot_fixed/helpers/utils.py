"""
وظائف مساعدة مشتركة
✅ إصلاح: استبدال r.get() المتزامن بنمط stale-while-revalidate
   → عند انتهاء الكاش: أعد القيمة القديمة فوراً + حدّث async في الخلفية
   → لا blocking على event loop نهائياً
"""
import asyncio
import time
import hashlib
...
# ── تحديث async في الخلفية (لا blocking) ────────────────────────────────

async def _refresh_bool(key: str, ttl: float):
    try:
...
async def _refresh_str(cache_key: str, redis_key: str):
    try:
...
# ── الكاش الذكي: stale-while-revalidate ─────────────────────────────────
...
def _bool_cached(key: str, ttl: float = _ENABLE_TTL) -> bool:
    ...
    if entry:
        ...
        if now - ts < ttl:
            return val
        # انتهى الكاش — أعد القيمة القديمة فوراً + حدّث async في الخلفية
        try:
            loop = asyncio.get_running_loop()
            loop.create_task(_refresh_bool(key, ttl))
        except RuntimeError:
            pass
        return val  # لا blocking على الإطلاق

    # أول مرة فقط — اجلب بشكل متزامن مرة واحدة ثم يُكمّل async
    try:
        val = bool(r.get(key))
    except Exception:
        val = False
...
def _str_cached(cache_key: str, redis_key: str) -> str | None:
    ...
    if entry:
        ...
        if now - ts < _RTXT_TTL:
            return val
        # انتهى الكاش — أعد القيمة القديمة + حدّث async في الخلفية
        try:
            loop = asyncio.get_running_loop()
            loop.create_task(_refresh_str(cache_key, redis_key))
        except RuntimeError:
            pass
        return val  # لا blocking
